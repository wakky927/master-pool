...
def ptv2t(img):
    ...
    s1, s2 = 9, 4.5  # 1st, 2nd search window
    ...
    for ii in range(npa):
        if pp01[ii].flag:
            result[ii][0] = pp0[pp01[ii].p0][1]
            result[ii][1] = pp0[pp01[ii].p0][0]
            result[ii][2] = pp1[pp01[ii].p1][1] - pp0[pp01[ii].p0][1]
            result[ii][3] = pp1[pp01[ii].p1][0] - pp0[pp01[ii].p0][0]
...
def main(args):
    ...
    for n in tqdm(range(1, 100)):
        data_list.popleft()
        tmp = np.loadtxt(in_dir + f"pp_{n}.csv", delimiter=',', skiprows=1)
        ...
        if r is None:
            ...
            np.savetxt(out_dir + f"{n-2}.csv", r, delimiter=',', header="x, y, dx, dy")
